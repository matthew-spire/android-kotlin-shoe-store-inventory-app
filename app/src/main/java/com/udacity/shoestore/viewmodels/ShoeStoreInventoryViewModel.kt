...
class ShoeStoreInventoryViewModel : ViewModel() {
    ...
    fun validateAndAddShoe(): Boolean {
        if (!validateShoeInputs()) return false

        val currentShoe = Shoe(shoeName, shoeSize, shoeCompany, shoeDescription)
        addShoe(currentShoe)

        return true
    }

    private fun validateShoeInputs(): Boolean {
        if (shoeName.isBlank() || shoeCompany.isBlank() || shoeSize.toString().isBlank() || shoeDescription.isBlank()) {
            return false
        }
